...
void ComputeJacobian(Eigen::MatrixXd &Jacobian, std::vector<Point3D> &points,
                     std::pair<std::vector<Point3D>, std::vector<SE3>> &Vertexs) {
    // 待优化的位姿节点数量
    int N_poses = Vertexs.second.size();
    // 3D坐标点的节点数量
    int N_points = points.size();
    //                                               Pose        Point1     Point2      PointN
    //                      /     Point1     |                  Jp1(2×3)    0(2×3)      0(2×3)  |
    //               Pose1         ...       |      J1(2×6)       ...      Jp2(2×3)      ...    |
    //                      \ PointN_points  |                   0(2×3)     0(2×3)     JpN(2×3) |
    //                      /     Point1     |                  Jp1(2×3)    0(2×3)      0(2×3)  |
    //               Pose2         ...       |      J2(2×6)       ...      Jp2(2×3)      ...    |
    //                      \ PointN_points  |                   0(2×3)     0(2×3)     JpN(2×3) |
    // JacobianPoint =              ⋮        |         ⋮           ⋮          ⋮           ⋮     |
    //                      /     Point1     |                  Jp1(2×3)    0(2×3)      0(2×3)  |
    //         PoseN_poses         ...       |      JN(2×6)       ...      Jp2(2×3)      ...    |
    //                      \ PointN_points  |                   0(2×3)     0(2×3)     JpN(2×3) | (2·N_poses·N_points × (6 + 3·N_points))
    Jacobian = Eigen::MatrixXd::Zero(2 * N_poses * N_points, 6 + 3 * N_points);
    if (debug_) {
        cout << "Jacobian : rows(" << Jacobian.rows() << ") cols(" << Jacobian.cols() << ")" << endl;
    }

//    Vertexs.first = Eigen::Vector3d::Ones(3,1);

    // 循环用的Jacobian，放在循环外部初始化
    Eigen::MatrixXd JacobianPose_i_(Eigen::MatrixXd::Zero(2, 3));
    //
    for (int j = 0; j < N_poses; ++j) {
        for (int i = 0; i < N_points; ++i) {

            // P' = R*P + t
            Eigen::MatrixXd R_ = Vertexs.second[j].block<3, 3>(0, 0);
            Eigen::MatrixXd t_ = Vertexs.second[j].block<3, 1>(0, 3);
            Eigen::Vector3d P_ = R_ * Vertexs.first[i] + t_;
            double x = P_(0, 0), y = P_(1, 0), z = P_(2, 0), z_2 = z * z;

            // Jacobian of pose : のf/のp 坐标导数
            JacobianPose_i_(0, 0) = -fx / z;
            JacobianPose_i_(0, 1) = 0;
            JacobianPose_i_(0, 2) = fx * x / z_2;

            JacobianPose_i_(1, 0) = 0;
            JacobianPose_i_(1, 1) = -fy / z;
            JacobianPose_i_(1, 2) = fy * y / z_2;
            JacobianPose_i_ *= R_;
            Jacobian.block<2, 3>(2 * j * N_points + 2 * i, 5 + 3 * i) = JacobianPose_i_;

//            if(j == 0)
            {
                Eigen::MatrixXd JacobianPose_j_(Eigen::MatrixXd::Zero(2, 6));
                // Jacobian of pose : のf/のT 位姿导数
                JacobianPose_j_(0, 0) = -1. / z * fx;
                JacobianPose_j_(0, 1) = 0;
                JacobianPose_j_(0, 2) = x / z_2 * fx;
                JacobianPose_j_(0, 3) = x * y / z_2 * fx;
                JacobianPose_j_(0, 4) = -(1 + (x * x / z_2)) * fx;
                JacobianPose_j_(0, 5) = y / z * fx;
                JacobianPose_j_(1, 0) = 0;
                JacobianPose_j_(1, 1) = -1. / z * fy;
                JacobianPose_j_(1, 2) = y / z_2 * fy;
                JacobianPose_j_(1, 3) = (1 + (y * y / z_2)) * fy;
                JacobianPose_j_(1, 4) = -x * y / z_2 * fy;
                JacobianPose_j_(1, 5) = -x / z * fy;

                Jacobian.block<2, 6>(2 * j * N_points + 2 * i, 0) = JacobianPose_j_;
            }
...
double ComputeReprojectionError(Eigen::VectorXd &ReprojectionError, std::vector<Point3D> &points3d,
    ...
    if (debug_) {
        cout << "ReprojectionError : rows(" << ReprojectionError.rows() << ") cols(" << ReprojectionError.cols() << ")"
             << endl;
    }
    for (int j = 0; j < N_poses; ++j) {
...
// 计算增量
Eigen::MatrixXd ComputeDeltaX(const Eigen::MatrixXd &JacobianPose, const Eigen::VectorXd &ReprojectionError) {
...
// 利用增量更新优化变量
void UpdateX(std::pair<std::vector<Point3D>, std::vector<SE3>> &Vertexs, Eigen::VectorXd delta_se3) {
    // 待优化的位姿节点数量
    int N_poses = Vertexs.second.size();
...
//            cout << delta_SE3_i << endl;
        ...
        // Update pose_i
        Vertexs.second[i] = delta_SE3_i * Vertexs.second[i];

        // Update point_j
        int N_points = Vertexs.first.size();
        for (int j = 0; j < N_points; ++j) {
            Vertexs.first[j] += delta_se3_i.t_;
        }
...
void
Gaussian_Newton(const int iterations, std::pair<std::vector<Point3D>, std::vector<SE3>> &Vertexs, vector<Point3D> &p3ds,
                vector<Point2D> &p2ds) {
    ...
    for (int i = 0; i < iterations; ++i) {
        // compute Jacobian
        ComputeJacobian(J_T, p3ds, Vertexs);
        // compute error
        cur_error = ComputeReprojectionError(Error, p3ds, Vertexs, p2ds);
        ...
        pre_error = cur_error;
        // computer delta_x
        Eigen::VectorXd delta_se3 = ComputeDeltaX(J_T, Error);
        if (debug_) {
            cout << "delta_se3 = " << delta_se3 << endl;
        }
        // 利用delta的值来停止迭代效果更好
...
int main(int argc, char **argv) {
    ...
    std::pair<std::vector<Point3D>, std::vector<SE3>> Vertexs = std::make_pair(VertexPoints3D, VertexPoses);
    for (int i = 0; i < Vertexs.first.size(); ++i) {
        if (fabs(Vertexs.first[i].x() - VertexPoints3D[i].x()) > 0.0000001 ||
            fabs(Vertexs.first[i].y() - VertexPoints3D[i].y()) > 0.0000001
            || fabs(Vertexs.first[i].z() - VertexPoints3D[i].z()) > 0.0000001) {
            cout << "Vertexs VertexPoints3D 不等于输入" << endl;
            return -1;
        }
    }

    // 高斯牛顿解非线性方程
    Gaussian_Newton(100, Vertexs, VertexPoints3D, Points2D);
    for (int i = 0; i < VertexPoses.size(); ++i) {
        cout << "VertexPoses[" << i << "] : rows(" << VertexPoses[i].rows() << ") cols(" << VertexPoses[i].cols() << ")"
             << endl;
        cout << VertexPoses[i] << endl;
    }
    return 0;
